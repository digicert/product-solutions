...
# Log first and last few characters of extracted data for verification (security-conscious logging)
if [ ${#CERT_DATA_ONLY} -gt 20 ]; then
    CERT_PREVIEW="${CERT_DATA_ONLY:0:10}...${CERT_DATA_ONLY: -10}"
    log_message "Certificate data preview: $CERT_PREVIEW"
else
    log_message "Certificate data too short for preview"
fi

if [ ${#KEY_DATA_ONLY} -gt 20 ]; then
    KEY_PREVIEW="${KEY_DATA_ONLY:0:10}...${KEY_DATA_ONLY: -10}"
    log_message "Private key data preview: $KEY_PREVIEW"
else
    log_message "Private key data too short for preview"
fi
...
log_message "=========================================="
log_message "Variable extraction completed successfully"
