...
VAULT_PRIVATE_KEYS_PATH="secret/private-keys"
...
get_f5xc_token_from_vault() {
    ...
    # Don't unset VAULT_TOKEN here as we'll need it to store the private key
    
    return 0
}

# Function to store private key in Vault and create reference file
store_private_key_in_vault() {
    local key_file_path="$1"
    local cert_name="$2"
    
    log_message "=========================================="
    log_message "Starting private key storage in Vault"
    log_message "=========================================="
    ...
    local key_name="${cert_name}-${timestamp}"
    local vault_path="${VAULT_PRIVATE_KEYS_PATH}/${key_name}"
    
    log_message "Vault storage path: $vault_path"
    
    # Verify we have a valid token
    if [ -z "$VAULT_TOKEN" ]; then
        log_message "ERROR: No Vault token available"
        return 1
    fi
    
    # Read the private key content (use base64 to handle special characters)
    local key_content_b64=$(cat "$key_file_path" | base64 -w 0)
    
    log_message "Key file size: $(stat -c%s "$key_file_path") bytes"
    log_message "Attempting to store in Vault..."
    
    # Store in Vault using base64 encoded content
    vault kv put "$vault_path" \
        private_key_b64="$key_content_b64" \
        certificate_name="$cert_name" \
        original_file="$(basename "$key_file_path")" \
        stored_at="$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
        stored_by="$(whoami)@$(hostname)" \
        tenant="$F5XC_TENANT" \
        namespace="$F5XC_NAMESPACE" > /dev/null 2>&1
    
    if [ $? -eq 0 ]; then
        log_message "Successfully stored private key in Vault at: $vault_path"
        
        # Create reference file
        local reference_file="${key_file_path}.vault-ref"
        cat > "$reference_file" << EOF
===============================================
PRIVATE KEY STORED IN VAULT
===============================================
This file is a reference to the private key that has been
securely stored in HashiCorp Vault for security purposes.

Original File: $(basename "$key_file_path")
Vault Path: $vault_path
Stored At: $(date -u +%Y-%m-%dT%H:%M:%SZ)
Certificate Name: $cert_name
F5XC Tenant: $F5XC_TENANT
F5XC Namespace: $F5XC_NAMESPACE

To retrieve the private key, use:
vault kv get -field=private_key_b64 "$vault_path" | base64 -d

Note: You must be authenticated with Vault and have
appropriate permissions to access this path.
===============================================
EOF
        
        log_message "Created reference file: $reference_file"
        chmod 644 "$reference_file"
        
        # Secure delete the original private key file
        if command -v shred &> /dev/null; then
            log_message "Securely deleting private key file..."
            # FIXED: Added -u flag to actually remove the file after shredding
            shred -vfzu -n 3 "$key_file_path" 2>&1 | while read line; do
                log_message "Secure delete: $line"
            done
        else
            log_message "Deleting private key file (simple deletion)..."
            rm -f "$key_file_path"
        fi
        
        # Verify deletion
        if [ ! -f "$key_file_path" ]; then
            log_message "✓ Private key file successfully removed from filesystem"
            log_message "✓ Private key is now only available in Vault at: $vault_path"
            return 0
        else
            log_message "WARNING: Failed to delete private key file from filesystem"
            return 1
        fi
    else
        log_message "ERROR: Failed to store private key in Vault"
        log_message "Debugging: Attempting simple test write..."
        
        # Try a simple test to debug
        if vault kv put secret/private-keys/debug-test-$(date +%s) test="true" > /dev/null 2>&1; then
            log_message "Debug: Simple write works, issue might be with key content"
            log_message "Debug: Key file size: $(stat -c%s "$key_file_path") bytes"
        else
            log_message "Debug: Cannot write to Vault at all - token might have expired"
        fi
        return 1
    fi
}
...
log_message "  VAULT_PRIVATE_KEYS_PATH: $VAULT_PRIVATE_KEYS_PATH"
...
# Variable to track upload success
...
if [ -z "$F5XC_TENANT" ] || [ -z "$F5XC_NAMESPACE" ] || [ -z "$F5XC_CERT_NAME" ] || [ -z "$F5XC_API_TOKEN" ]; then
    ...
    if [ ! -f "$CRT_FILE_PATH" ]; then
        ...
        # If upload was successful, store the private key in Vault and delete from filesystem
        if [ "$UPLOAD_SUCCESS" = true ] && [ -f "$KEY_FILE_PATH" ]; then
            log_message "Certificate upload successful, proceeding to secure private key..."
            
            if store_private_key_in_vault "$KEY_FILE_PATH" "$F5XC_CERT_NAME"; then
                log_message "✓ Private key successfully secured in Vault"
            else
                log_message "WARNING: Failed to store private key in Vault, key remains on filesystem"
            fi
        elif [ "$UPLOAD_SUCCESS" = false ]; then
            log_message "Certificate upload failed, keeping private key on filesystem for troubleshooting"
        fi
    fi
fi
