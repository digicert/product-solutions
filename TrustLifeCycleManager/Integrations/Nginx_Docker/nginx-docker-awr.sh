...
# ----------------------------------------
# Deploy renewed certificate/key into the nginx Docker container and reload.
#
# Exit-code hygiene note:
#   The TLM AWR agent grades the post-script by scanning captured output for
#   ANY recognizable exit-code pattern. nginx writes its reload confirmation
#   (e.g. "NNN#NNN: signal process started") to STDERR even on success, and
#   those bare "NNN#NNN" numbers can be misread as a status -> false "Failed".
#
#   To keep the graded stream unambiguous we:
#     1. Send nginx stdout to /dev/null and its stderr to a temp file.
#     2. Capture the REAL exit code ourselves via $? (wrapped in set +e/-e so
#        the template's flow controls it, not errexit/pipefail mid-statement).
#     3. Replay nginx's stderr text back to the log ONLY on genuine failure,
#        so real errors stay diagnosable but success leaks nothing scrapeable.
#     4. exit non-zero on failure (the template ends on `exit 0`, so a failure
#        here must exit itself or it would be masked as success).

# --- Tunables (override via AWR args if desired) --------------------------
CONTAINER_NAME="${ARGUMENT_1:-webserver}"        # docker container running nginx
DEST_DIR="${ARGUMENT_2:-/etc/digicert/certs}"    # host path for deployed cert/key
COMPOSE_DIR="${ARGUMENT_3:-/usr/local/docker}"   # working dir for the docker stack
# --------------------------------------------------------------------------

log_message "nginx deploy config: container='$CONTAINER_NAME' dest='$DEST_DIR' compose_dir='$COMPOSE_DIR'"

# Preconditions
if ! command -v docker >/dev/null 2>&1; then
    log_message "ERROR: docker command not found on PATH"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
fi
if [ ! -f "$CRT_FILE_PATH" ] || [ ! -f "$KEY_FILE_PATH" ]; then
    log_message "ERROR: certificate or key file missing (crt='$CRT_FILE_PATH' key='$KEY_FILE_PATH')"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
fi

# Deploy cert + key to the host directory bind-mounted into the container
if ! mkdir -p "$DEST_DIR"; then
    log_message "ERROR: could not create destination directory '$DEST_DIR'"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
fi
cp "$CRT_FILE_PATH" "$DEST_DIR/server.crt" && cp "$KEY_FILE_PATH" "$DEST_DIR/server.key"
if [ $? -ne 0 ]; then
    log_message "ERROR: failed to copy certificate/key into '$DEST_DIR'"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
fi
...
log_message "Certificate deployed to $DEST_DIR/server.crt (644) and server.key (600)"

# Temp file to capture nginx stderr so we can replay it only on failure
NGINX_ERR=$(mktemp)
trap 'rm -f "$NGINX_ERR"' EXIT

cd "$COMPOSE_DIR" 2>/dev/null || log_message "NOTE: could not cd to '$COMPOSE_DIR', continuing from current directory"

# 1) Validate the running config first; do not reload a broken nginx.
set +e
docker exec "$CONTAINER_NAME" nginx -t >/dev/null 2>"$NGINX_ERR"
TEST_RC=$?
set -e
if [ "$TEST_RC" -ne 0 ]; then
    log_message "ERROR: nginx configuration test failed (rc=$TEST_RC)"
    while IFS= read -r line; do log_message "  nginx: $line"; done < "$NGINX_ERR"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
fi
...
# 2) Reload. Capture the true exit status; keep the success notice out of any
#    scraped stream, replay stderr only if the reload genuinely fails.
set +e
docker exec "$CONTAINER_NAME" nginx -s reload >/dev/null 2>"$NGINX_ERR"
RELOAD_RC=$?
set -e
if [ "$RELOAD_RC" -ne 0 ]; then
    log_message "ERROR: nginx reload failed (rc=$RELOAD_RC)"
    while IFS= read -r line; do log_message "  nginx: $line"; done < "$NGINX_ERR"
    log_message "Custom script section FAILED"
    log_message "=========================================="
    exit 1
...
log_message "nginx reloaded successfully in container '$CONTAINER_NAME' (rc=0)"
# ----------------------------------------
# END CUSTOM LOGIC
