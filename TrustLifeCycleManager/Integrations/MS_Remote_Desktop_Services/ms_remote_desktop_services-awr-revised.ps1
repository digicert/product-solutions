...
if (-not $isRdsServer) {
    Write-LogMessage "Get-RDServer returned no result - this host is not an RDS server; skipping RDS configuration."
}
elseif ([string]::IsNullOrEmpty($PFX_FILE_PATH) -or -not (Test-Path $PFX_FILE_PATH)) {
