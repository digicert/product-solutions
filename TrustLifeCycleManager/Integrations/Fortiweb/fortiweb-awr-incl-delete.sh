...
# Configuration
LEGAL_NOTICE_ACCEPT="true"  # Set to "true" to accept the legal notice and enable script execution
...
if [ -z "$ARGUMENT_1" ]; then
    ...
    if [ -z "$ARGUMENT_2" ]; then
        ...
        if [ -f "$CRT_FILE_PATH" ] && [ -f "$KEY_FILE_PATH" ]; then
            ...
            # ============================================================
            # CHECK FOR EXISTING CERTIFICATE AND DELETE IF FOUND (RENEWAL)
            # ============================================================
            # FortiWeb names certificates based on the Common Name (CN) from
            # the certificate, so we extract it using openssl.
            # We list all certificates and search for a match, since dots in
            # the CN (e.g. tls.guru) can cause issues with direct GET by name.
            
            FORTIWEB_CERT_NAME=$(openssl x509 -in "$CRT_FILE_PATH" -noout -subject 2>/dev/null | sed -n 's/.*CN\s*=\s*//p' | sed 's/\s*$//')
            if [ -z "$FORTIWEB_CERT_NAME" ]; then
                log_message "WARNING: Could not extract Common Name from certificate - skipping renewal check"
                log_message "Proceeding directly with import (may fail if certificate already exists)"
            else
                log_message "Extracted Common Name from certificate: $FORTIWEB_CERT_NAME"
                
                log_message "Listing all certificates on FortiWeb to check for existing cert..."
                
                LIST_ENDPOINT="https://${FORTIWEB_URL}:8443/api/v2.0/system/certificate.local"
                log_message "GET ${LIST_ENDPOINT}"
                
                LIST_RESPONSE=$(curl -k --location -g --request GET "$LIST_ENDPOINT" \
                ...
                log_message "List Response HTTP Status: $LIST_HTTP_STATUS"
                
                # Check if our certificate name appears in the list
                CERT_EXISTS="false"
                if [ "$LIST_HTTP_STATUS" == "200" ]; then
                    if echo "$LIST_RESPONSE_BODY" | grep -q "\"${FORTIWEB_CERT_NAME}\""; then
                        CERT_EXISTS="true"
                        log_message "Certificate '${FORTIWEB_CERT_NAME}' found in FortiWeb certificate list - this is a RENEWAL"
                    else
                        log_message "Certificate '${FORTIWEB_CERT_NAME}' not found in FortiWeb certificate list - this is an INITIAL IMPORT"
                    fi
                else
                    log_message "WARNING: Could not list certificates from FortiWeb (HTTP Status: $LIST_HTTP_STATUS)"
                    log_message "Proceeding with import attempt..."
                fi
                
                if [ "$CERT_EXISTS" == "true" ]; then
                    log_message "Deleting existing certificate before re-import..."
                    
                    # FortiWeb uses the CMDB path with mkey query parameter for deletion
                    DELETE_ENDPOINT="https://${FORTIWEB_URL}:8443/api/v2.0/cmdb/system/certificate.local?mkey=${FORTIWEB_CERT_NAME}"
                    log_message "DELETE ${DELETE_ENDPOINT}"
                    
                    DELETE_RESPONSE=$(curl -k --location -g --request DELETE "$DELETE_ENDPOINT" \
                        --header "Authorization: $AUTH_TOKEN" \
                        --header 'Accept: application/json' \
                        --write-out "\nHTTP_STATUS:%{http_code}" \
                        --silent \
                    ...
                    log_message "Delete Response HTTP Status: $DELETE_HTTP_STATUS"
                    log_message "Delete Response Body: $DELETE_RESPONSE_BODY"
                    
                    if [ "$DELETE_HTTP_STATUS" == "200" ]; then
                        log_message "SUCCESS: Existing certificate '${FORTIWEB_CERT_NAME}' deleted successfully"
                        log_message "Waiting 2 seconds for FortiWeb to process deletion..."
                        sleep 2
                    else
                        log_message "ERROR: Failed to delete existing certificate '${FORTIWEB_CERT_NAME}' (HTTP Status: $DELETE_HTTP_STATUS)"
                        log_message "Delete Response: $DELETE_RESPONSE_BODY"
                        log_message "The certificate may be in use by a server policy or other FortiWeb object."
                        exit 1
                    fi
                fi
            fi
            ...
            # Perform the certificate upload using curl
            log_message "Executing certificate upload to FortiWeb..."
            
            CURL_RESPONSE=$(curl -k --location -g --request POST "$API_ENDPOINT" \
                --header "Authorization: $AUTH_TOKEN" \
                --header 'Accept: application/json' \
                --header 'Content-Type: multipart/form-data' \
                --form "certificateFile=@${CRT_FILE_PATH}" \
            ...
            if [ "$HTTP_STATUS" == "200" ] || [ "$HTTP_STATUS" == "201" ]; then
                ...
                log_message "FortiWeb certificate import completed"
                
                # Optional: Parse response for certificate ID or name if returned
                ...
                if [ "$HTTP_STATUS" == "401" ]; then
                    ...
                    log_message "Bad request - please verify the certificate and key files are valid"
