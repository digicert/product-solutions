...
# ============================================================================
# CHECK FOR EXISTING CERTIFICATE AND DELETE IF FOUND (RENEWAL SUPPORT)
# ============================================================================

log_message "Checking if certificate '${CERT_NAME}' already exists on FortiGate..."

FORTIGATE_CHECK_URL="https://${FORTIGATE_URL}/api/v2/cmdb/vpn.certificate/local/${CERT_NAME}"
log_message "GET ${FORTIGATE_CHECK_URL}"

CHECK_RESPONSE=$(curl -k --silent --show-error --write-out "\nHTTP_STATUS:%{http_code}" \
    --location --request GET "$FORTIGATE_CHECK_URL" \
    --header "Authorization: Bearer ${BEARER_TOKEN}" \
    --header "Content-Type: application/json" 2>&1)

CHECK_HTTP_STATUS=$(echo "$CHECK_RESPONSE" | grep -oP 'HTTP_STATUS:\K[0-9]+')
CHECK_RESPONSE_BODY=$(echo "$CHECK_RESPONSE" | sed 's/HTTP_STATUS:[0-9]*$//')

log_message "Check Response HTTP Status: $CHECK_HTTP_STATUS"

if [ "$CHECK_HTTP_STATUS" = "200" ]; then
    log_message "Certificate '${CERT_NAME}' already exists on FortiGate - this is a RENEWAL"
    log_message "Deleting existing certificate before re-import..."

    FORTIGATE_DELETE_URL="https://${FORTIGATE_URL}/api/v2/cmdb/vpn.certificate/local/${CERT_NAME}"
    log_message "DELETE ${FORTIGATE_DELETE_URL}"

    DELETE_RESPONSE=$(curl -k --silent --show-error --write-out "\nHTTP_STATUS:%{http_code}" \
        --location --request DELETE "$FORTIGATE_DELETE_URL" \
        --header "Authorization: Bearer ${BEARER_TOKEN}" \
        --header "Content-Type: application/json" 2>&1)

    DELETE_HTTP_STATUS=$(echo "$DELETE_RESPONSE" | grep -oP 'HTTP_STATUS:\K[0-9]+')
    DELETE_RESPONSE_BODY=$(echo "$DELETE_RESPONSE" | sed 's/HTTP_STATUS:[0-9]*$//')
    ...
    if [ "$DELETE_HTTP_STATUS" = "200" ]; then
        log_message "SUCCESS: Existing certificate '${CERT_NAME}' deleted successfully"
        log_message "Waiting 2 seconds for FortiGate to process deletion..."
        sleep 2
    elif [ "$DELETE_HTTP_STATUS" = "424" ]; then
        log_message "ERROR: Certificate '${CERT_NAME}' is in use by a FortiGate object (e.g. HTTPS admin, VPN, etc.)"
        log_message "The certificate cannot be deleted while it is referenced by other configurations."
        log_message "Please remove the certificate binding from the FortiGate configuration first, or use a different certificate name."
        log_message "Delete Response: $DELETE_RESPONSE_BODY"
        exit 1
    else
        log_message "ERROR: Failed to delete existing certificate '${CERT_NAME}' (HTTP Status: $DELETE_HTTP_STATUS)"
        log_message "Delete Response: $DELETE_RESPONSE_BODY"
        exit 1
    fi
else
    log_message "Certificate '${CERT_NAME}' does not exist on FortiGate - this is an INITIAL IMPORT"
fi
