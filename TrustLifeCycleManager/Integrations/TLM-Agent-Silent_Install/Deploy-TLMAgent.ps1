...
.DESCRIPTION
    ...
    - Optional cleanup of GPO-set environment variables after activation
...
.PARAMETER CleanupEnvVars
    Switch to remove GPO-set environment variables (DC_API_KEY, TLM_BUSINESS_UNIT, TLM_AGENT_NAME, DCONE_HOST)
...
.EXAMPLE
    .\Deploy-TLMAgent.ps1 -Action InstallAndActivate -CleanupEnvVars
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [switch]$CleanupEnvVars,

    [Parameter(Mandatory = $false)]
    [string]$LogFile = "\\dc\TLMAgentDeploy\logs\$($env:COMPUTERNAME)_tlm.log"
)
...
# Read directly from the registry (Machine-level) first to avoid stale
# process environment issues when launched from cmd.exe or scheduled tasks.
# Falls back to $env: (process-level) as a safety net.
if (-not $ApiKey) {
    $ApiKey = [System.Environment]::GetEnvironmentVariable('DC_API_KEY', [System.EnvironmentVariableTarget]::Machine)
    if (-not $ApiKey) { $ApiKey = $env:DC_API_KEY }
}

if (-not $BusinessUnitId) {
    $BusinessUnitId = [System.Environment]::GetEnvironmentVariable('TLM_BUSINESS_UNIT', [System.EnvironmentVariableTarget]::Machine)
    if (-not $BusinessUnitId) { $BusinessUnitId = $env:TLM_BUSINESS_UNIT }
}

if (-not $AgentName) {
    $AgentName = [System.Environment]::GetEnvironmentVariable('TLM_AGENT_NAME', [System.EnvironmentVariableTarget]::Machine)
    if (-not $AgentName) { $AgentName = $env:TLM_AGENT_NAME }
}

if (-not $DcOneHost) {
    $DcOneHost = [System.Environment]::GetEnvironmentVariable('DCONE_HOST', [System.EnvironmentVariableTarget]::Machine)
    if (-not $DcOneHost) { $DcOneHost = $env:DCONE_HOST }
}
...
function Remove-DeploymentEnvVars {
    <#
    .SYNOPSIS
        Removes GPO-set environment variables used for TLM Agent deployment.
    .DESCRIPTION
        Cleans up machine-level environment variables that were configured via GPO
        for agent deployment. This prevents sensitive data (e.g. API keys) from
        persisting on the system after successful activation.
    #>
    
    Write-Log "==========================================" -Level INFO
    Write-Log "Cleaning Up Environment Variables" -Level INFO
    Write-Log "==========================================" -Level INFO
    
    $envVarsToRemove = @('DC_API_KEY', 'TLM_BUSINESS_UNIT', 'TLM_AGENT_NAME', 'DCONE_HOST')
    $removedCount = 0
    ...
    foreach ($varName in $envVarsToRemove) {
        ...
        if ($currentValue) {
            try {
                ...
                Write-Log "Removed system environment variable: $varName" -Level SUCCESS
                $removedCount++
            }
            catch {
                Write-Log "Failed to remove environment variable '$varName': $_" -Level WARNING
            }
        }
        else {
            Write-Log "Environment variable '$varName' not set at machine level - skipping" -Level INFO
        }
    }
    
    # Also remove from current process environment so they are not available in this session
    foreach ($varName in $envVarsToRemove) {
        if ([System.Environment]::GetEnvironmentVariable($varName, [System.EnvironmentVariableTarget]::Process)) {
            [System.Environment]::SetEnvironmentVariable($varName, $null, [System.EnvironmentVariableTarget]::Process)
        }
    }
    
    Write-Log "Environment variable cleanup complete. Removed $removedCount variable(s)." -Level INFO
    
    if ($removedCount -gt 0) {
        Write-Log "NOTE: If these variables are managed by GPO, ensure the GPO is updated" -Level WARNING
        Write-Log "or uses WMI filtering to exclude activated machines, otherwise the" -Level WARNING
        Write-Log "variables will be recreated on the next Group Policy refresh." -Level WARNING
    }
...
try {
    ...
    Write-Log "Cleanup Environment Variables: $CleanupEnvVars" -Level INFO
    ...
    if ($Action -eq 'Activate' -or $Action -eq 'InstallAndActivate') {
        ...
        # Clean up environment variables after successful activation
        if ($activationSuccess -and $CleanupEnvVars) {
            $envVarsRemoved = Remove-DeploymentEnvVars
        }
    ...
    if ($CleanupEnvVars -and $activationSuccess) {
        Write-Log "Environment Cleanup: $envVarsRemoved variable(s) removed" -Level SUCCESS
    }
